...
{
	/* 4.14 lacks ktime_get_tai_fast_ns(), so use the available TAI clock. */
			...
			u32 num_args)
{
	...
	for (i = 0; i < fmt_size; i++) {
		...
		if (fmt[i] == 'p') {
			...
			err = probe_kernel_read(tmp_buf, unsafe_ptr, copy_size);
			if (err < 0)
...
/* struct bpf_timer is available to value types of array and hash maps.
 * The verifier will allow programs to use timers only if the map value
 * BTF has a struct bpf_timer field and the map implementation exposes
 * enough space in each value to keep timer state out of line.
 *
	   ...
	   u64, flags)
{
	...
	t = kmalloc_node(sizeof(*t), GFP_ATOMIC | __GFP_NOWARN, map->numa_node);
	if (!t) {
		ret = -ENOMEM;
		goto out;
	}
